...
@router.get("/{team_id}")
async def read(team_id: uuid.UUID, session: AsyncSession = Depends(get_session)) -> Team:
    return await team.read(
        session=session,
        team_id=team_id
    )
...
async def create(data: TeamCreate, session: AsyncSession = Depends(get_session)) -> Team:
...
async def update(team_id: uuid.UUID, data: TeamUpdate, session: AsyncSession = Depends(get_session)) -> Team:
...
async def delete(team_id: uuid.UUID, session: AsyncSession = Depends(get_session)) -> None:
